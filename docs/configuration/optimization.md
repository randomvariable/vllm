...
- Increase `gpu_memory_utilization`. vLLM pre-allocates GPU cache using this percentage of memory. By increasing utilization, you can provide more KV cache space. On unified-memory devices, where a fraction of total device memory is a poor control, raise the absolute [`--gpu-memory-utilization-gb`](./conserving_memory.md#gpu-memory-budget) budget instead.
