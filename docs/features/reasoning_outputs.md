...
answer tokens often want different entropy regardless of uncertainty. It
overrides `temperature` (and any ReSET decision), but **only** after the model
has left the reasoning block.
...
- A request that never enters reasoning never takes the override. It stays on
  `temperature`, or on ReSET if it is enabled.
...
alone is a configuration error rather than a silent no-op. It does not interact
with `thinking_token_budget` or `reasoning_answer_reserve`, which control
*where* the reasoning block ends.
