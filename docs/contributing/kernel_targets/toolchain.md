...
**Confirmed (in tree).** The tree is not uniform. The cross build compiles the
vLLM wheel with `TORCH_CUDA_ARCH_LIST="12.0 12.1a"`
(`homelab/spark-cross.Dockerfile`), but the GGUF plugin wheel and runtime stage
set `12.1a` alone, as does the native `homelab/spark.Dockerfile` throughout.
Most of what ships is therefore SM121-exclusive.
