...
# ---------------------------------------------------------------------------
# ATOM-ported in-process NUMA bind (sched_setaffinity + numa_set_preferred)
# ---------------------------------------------------------------------------
# Adapted from ROCm/ATOM atom/utils/numa_utils.py. This is a lighter-weight
# alternative to the numactl-wrapper approach above: it binds the current
# process in-place without spawning a subprocess. Useful when numactl is
# unavailable or when the operator prefers explicit in-process control.
#
# Gated on VLLM_NUMA_BIND (default off). Called once at worker init.
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        try:
            ...
            if phys >= len(handles):
                return None
            node = int(
                amdsmi.amdsmi_topo_get_numa_node_number(handles[phys]))
            return node if node >= 0 else None
        finally:
            amdsmi.amdsmi_shut_down()
    except Exception as e:
        logger.debug("amdsmi NUMA query failed for gpu %d: %s", gpu_id, e)
        return None
...
def _query_node_sysfs(gpu_id: int) -> int | None:
    ...
    *_VISIBLE_DEVICES permutation can break -- so this is only used when
    amdsmi is unavailable. Prefer the explicit ``VLLM_NUMA_NODE`` list on
    such setups.
    """
    import glob
    phys = _physical_index(gpu_id)
...
def _resolve_node_in_process(gpu_id: int) -> int | None:
    """Explicit ``VLLM_NUMA_NODE`` wins; else auto-detect (amdsmi -> sysfs)."""
    explicit = [x for x in envs.VLLM_NUMA_NODE.split(",") if x.strip() != ""]
    if explicit:
...
def _set_preferred_memory_in_process(node: int) -> None:
    """Best-effort memory binding via libnuma ``numa_set_preferred``.

    If libnuma is absent, first-touch on the pinned CPUs still lands memory
    on the local node, so this is an optimization, not a requirement.
    """
    libnuma = get_libnuma()
    if libnuma is None or libnuma.numa_available() < 0:
        return
...
def numa_bind_in_process(gpu_id: int, label: str = "") -> None:
    """Bind the current process to its GPU's NUMA-local cores and memory.

    No-op unless ``VLLM_NUMA_BIND`` is enabled. Must run before any large
    allocation / native (e.g. RDMA) thread spawn so the affinity mask is
    inherited by child threads and Linux first-touch places memory on the
    node. The node's CPUs are intersected with the current affinity so an
    existing container cpuset is respected. On failure it warns and returns
    (never raises), so a broken NUMA config degrades gracefully.

    Adapted from ROCm/ATOM ``atom.utils.numa_utils.numa_bind_to_node``.
    """
    if not envs.VLLM_NUMA_BIND:
        return
    tag = f" ({label})" if label else ""
    try:
        node = _resolve_node_in_process(gpu_id)
        if node is None or node < 0:
            raise RuntimeError(
                f"could not resolve NUMA node for gpu {gpu_id}")
        cpus = _node_cpus(node) & os.sched_getaffinity(0)
        if not cpus:
            raise RuntimeError(
                f"NUMA node {node} has no CPUs allowed by "
                f"the current affinity")
        os.sched_setaffinity(0, cpus)
        _set_preferred_memory_in_process(node)
        logger.info("NUMA bind%s: gpu=%d -> node %d (%d cores)", tag, gpu_id,
                    node, len(cpus))
    except Exception as e:
        msg = (
            f"NUMA bind{tag} failed for gpu {gpu_id}: {e}. "
            f"In docker add --cap-add SYS_NICE, or set VLLM_NUMA_NODE "
            f"explicitly. Continuing without NUMA binding.")
        logger.warning(msg)
