...
def _warmup_eagle_prepare_next_token_kernel(vllm_config: VllmConfig) -> None:
    from vllm.v1.spec_decode.utils import get_eagle_prepare_next_token_kernel

    get_eagle_prepare_next_token_kernel().warmup(vllm_config)


def _warmup_eagle_prepare_inputs_kernel(vllm_config: VllmConfig) -> None:
    from vllm.v1.spec_decode.utils import get_eagle_prepare_inputs_kernel

    get_eagle_prepare_inputs_kernel().warmup(vllm_config)
...
def _warmup_eagle_step_slot_mapping_metadata_kernel(
    ...
    from vllm.v1.spec_decode.utils import get_eagle_step_slot_mapping_kernel

    get_eagle_step_slot_mapping_kernel().warmup(
        vllm_config,
