...
def kernel_warmup(worker: "Worker", *, process_local_only: bool = False):
    ...
    if warmed_tensor_fp8:
        logger.info(
            "Warmed up %d B12X tensor FP8 linear GEMM signatures.", warmed_tensor_fp8
        )
    warmed_mla_bmm = warmup_b12x_mla_mxfp8_bmm(worker.get_model())
    if warmed_mla_bmm:
        logger.info("Warmed up %d B12X MLA MXFP8 BMM variants.", warmed_mla_bmm)
    mla_query_warmup_sizes = sorted(
        {1, *(size for size in cudagraph_capture_sizes if 1 <= size <= 32)}
    )
    warmed_mla_query = warmup_fused_mla_query(
        worker.get_model(), m_values=mla_query_warmup_sizes
    )
    if warmed_mla_query:
        logger.info(
            "Warmed up %d fused MLA BF16/MXFP8 query variants.", warmed_mla_query
        )
