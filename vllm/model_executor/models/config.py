...
class Gemma4Config(VerifyAndUpdateConfig):
    ...
    def verify_and_update_config(vllm_config: "VllmConfig") -> None:
        ...
        if (
            ...
            logger.info(
                "Gemma4 model has heterogeneous head dimensions %s with "
                "NVFP4 KV cache on CC 12.x. Routing to FLASHINFER (FA2 "
                "VO-split asymmetric head-dim kernel) instead of "
                "TRITON_ATTN.",
                head_dims,
            )
            return
