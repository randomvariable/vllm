...
class CacheConfig:
    ...
    utilization. If neither this nor `gpu_memory_utilization_gb` is specified,
    will use the default value of 0.92. This is a per-instance limit, and only
    ...
    utilization to 0.5 for each instance. Mutually exclusive with
    `gpu_memory_utilization_gb`."""
    gpu_memory_utilization_gb: float | None = Field(
        default=None, gt=0, allow_inf_nan=False
    )
    """Absolute per-worker GPU memory budget in GiB, as an alternative to the
    fractional `gpu_memory_utilization`. It is the *total* engine-resident
    target for this worker's device -- model weights, persistent non-KV state,
    the profiled activation peak, captured CUDA graphs, the KV cache, and any
    frontend GPU reservations all fit inside it -- not extra memory taken on
    top of what is already allocated.

    This is primarily useful on unified-memory devices (e.g. NVIDIA GB10 /
    DGX Spark, AMD Strix Halo), where a fraction of total device memory is a
    poor control because the CPU and GPU share one pool. Mutually exclusive
    ...
    """Explicit KV cache capacity per GPU in bytes. By default, this is set to
    None and vLLM infers the KV cache size from the active GPU memory budget.

    This differs in kind from the memory budgets: `gpu_memory_utilization` and
    `gpu_memory_utilization_gb` bound *whole-engine* memory and let profiling
    derive the KV cache from what is left over, whereas
    `kv_cache_memory_bytes` sizes *only* the KV cache and skips memory
    profiling entirely. Setting it therefore ignores `gpu_memory_utilization`,
    and it cannot be combined with `gpu_memory_utilization_gb`, whose
    whole-engine target profiling is what enforces."""
    ...
    @model_validator(mode="after")
    def _resolve_gpu_memory_budget(self) -> "CacheConfig":
        """Resolve the fractional and absolute GPU memory budgets.

        Exactly one control is active after validation. The two are mutually
        exclusive with no precedence rule, so a caller that sets both always
        gets an error rather than having one of them silently discarded.

        Unlike `_apply_block_size_default`, this validator sets no derived
        flag and is idempotent in every input state, so it is deliberately
        *not* guarded against pydantic re-running it when `CacheConfig` is
        nested in another model. Re-running is what lets the exclusion checks
        catch a second control set after construction; skipping them on
        revalidation would let both controls through.

        Raises:
            ValueError: If both `gpu_memory_utilization` and
                `gpu_memory_utilization_gb` are set, or if
                `gpu_memory_utilization_gb` is combined with
                `kv_cache_memory_bytes`.
        """
        if (
            self.gpu_memory_utilization is not None
            and self.gpu_memory_utilization_gb is not None
        ):
            raise ValueError(
                "gpu_memory_utilization and gpu_memory_utilization_gb are "
                "mutually exclusive: the first is a fraction of total device "
                "memory, the second an absolute per-worker GiB budget. Got "
                f"gpu_memory_utilization={self.gpu_memory_utilization} and "
                f"gpu_memory_utilization_gb={self.gpu_memory_utilization_gb}. "
        ...
        if (
            ...
            raise ValueError(
                "gpu_memory_utilization_gb cannot be combined with "
                "kv_cache_memory_bytes. gpu_memory_utilization_gb is a "
                "whole-engine budget enforced by memory profiling, while "
                "kv_cache_memory_bytes sizes only the KV cache and skips "
                "profiling, which would bypass that enforcement. Use "
                "gpu_memory_utilization with kv_cache_memory_bytes, or drop "
                "kv_cache_memory_bytes."
            )

        if (
            self.gpu_memory_utilization is None
            and self.gpu_memory_utilization_gb is None
        ):
