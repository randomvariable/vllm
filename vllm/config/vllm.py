...
class VllmConfig:
    ...
    def __post_init__(self):
        ...
        # On SM120, the breakable cudagraph remains the supported PIECEWISE
        # path. On tested SM121/GB10 Ray configurations, the compiled PIECEWISE
        # path is required for correctness; the breakable path can corrupt
        # graph replay outputs.
        # Auto-enable only for the known-good device path unless the user has
        # explicitly set the env var.
        if (
            self.model_config is not None
            and "VLLM_USE_BREAKABLE_CUDAGRAPH" not in os.environ
            and _should_auto_enable_deepseek_v4_breakable_cudagraph(
                self.model_config
            )
        ):
