...
class DFlashProposer(SpecDecodeBaseProposer):
    ...
    def _get_dflash_context_slot_mapping(
        ...
        gids = self._draft_kv_gids()
        if not self._draft_layer_to_kv_cache_gid or len(gids) == 1:
            return self._slot_mapping_buffers_by_gid[gids[0]][0][:num_context]
        return {
    ...
    def set_inputs_first_pass(
        ...
        # Rejected-token counts are GPU-only. Preserve upstream's conservative
        # CPU bound so metadata builders do not need a device synchronization.
