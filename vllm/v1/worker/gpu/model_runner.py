...
from vllm.v1.worker.gpu.spec_decode.utils import DraftTokensHandler, limit_draft_tokens
    ...
    def sample(
        ...
        with record_function_or_nullcontext(f"vllm:v2/target/{phase}/logits"):
            if self.batch_sharder is not None:
                # Shard inputs along the batch dimension to sample across TP ranks.
        ...
        if (
            ...
            and global_input_batch.num_draft_tokens_per_req is not None
        ):
            num_bonus = self.model_state.num_new_sampled_tokens_per_step
            num_logits = (
                global_input_batch.cu_num_logits[1:]
                - global_input_batch.cu_num_logits[:-1]
            )
            online_sts.record(
                global_input_batch.idx_mapping,
                num_sampled[: global_input_batch.num_reqs] - num_bonus,
