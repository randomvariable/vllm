...
class DFlashSpeculator(DraftModelSpeculator):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self.requires_non_causal = cast(
            Any, dflash_has_any_non_causal
        )(self.draft_model_config.hf_config)
    ...
    def set_attn(
        ...
        if hasattr(self.model, "get_draft_kv_cache_layer_names"):
            layer_names = cast(Any, self.model).get_draft_kv_cache_layer_names()
            name_to_gid = {
                ln: gid
            ...
            if hasattr(self.model, "get_draft_attn_causal"):
                self._group_causal = {
                    ...
                    for name, layer_causal in zip(
                        layer_names, cast(Any, self.model).get_draft_attn_causal()
                    )
                }
    ...
    def propose(
        ...
        if aux_hidden_states:
            hidden_states = cast(Any, self.model).combine_hidden_states(
        ...
        if dummy_run and skip_attn_for_dummy_run:
            ...
            cast(Any, self.model).precompute_and_store_context_kv(
...
class PrepareDflashInputsKernel(VllmJitKernel[Any]):
    ...
    def get_warmup_keys(
        ...
        *,
        ...
        if not group_ids:
            group_ids = list(range(len(input_block_tables)))
        group_inputs = [
            (int(input_block_tables[gid].stride(0)), kernel_block_sizes[gid])
            for gid in group_ids
            if gid < len(input_block_tables) and gid < len(kernel_block_sizes)
        ]
        if not group_inputs:
            return []

        keys: list[PrepareDflashInputsKernel.CompileKey] = []
        for block_table_stride, block_size in group_inputs:
            keys.extend(
                self._trace_dispatch(self.dispatch)(
                    ...
                    grid_num_reqs=[
                        n for n in self._GRID_NUM_REQS if n <= speculator.max_num_reqs
                    ],
    ...
    def __call__(
        ...
        cast(Any, self.kernel)[(num_reqs, num_blocks)](
