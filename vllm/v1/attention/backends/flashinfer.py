...
class FlashInferBackend(AttentionBackend):
    ...
    def supported_kv_cache_layouts(cls) -> tuple[KVCacheLayout, ...] | None:
        ...
        # NVFP4 KV on consumer Blackwell (sm120/sm121, FA2 path): each K/V side
        # of the cache packs [data | scale] regions carved out of the side's
        # byte range, which is only byte-coherent when each side's heads own
        # one contiguous region per page, i.e. a head-major layout. Under a
        # token-major layout the side-region offsets land inside the other
        # side's data and silently corrupt KV.
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if (
            ...
            raise ValueError(
                ...
                f"layout is "
                f"{get_flashinfer_layout_string(self.kv_cache_layout)!r}. Unset "
    ...
    def build(
        ...
        force_prefill = False
        if causal or self.use_dedicated_xqa:
        ...
        if num_prefill_tokens > 0:
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    kv_cache_for_fi: tuple[torch.Tensor, torch.Tensor]
                    if self.is_kvcache_nvfp4:
