...
def _is_lds_overflow_error(exc: BaseException) -> bool:
    """True if the exception is an LDS/shared-memory overflow from Triton."""
    if _TritonOutOfResources is not None and isinstance(
        exc, _TritonOutOfResources
    ):
        return True
    msg = str(exc).lower()
...
class RocmAiterUnifiedAttentionImpl(RocmAttentionImpl):
    ...
    def _forward_triton_unified(
        ...
        """In-tree Triton unified-attention fallback.

        Used for non-causal cross-attention (ENCODER_DECODER, e.g. Whisper)
        and when the AITER 3D kernel exceeds the device LDS/shared-memory
        limit (latched for the rest of the process).
        """
        ...
        if attn_metadata.causal and not self._aiter_lds_overflow:
            try:
                ...
                if not _is_lds_overflow_error(exc):
                    raise
                logger.warning_once(
                    "AITER unified-attention 3D kernel exceeds the LDS/"
                    "shared-memory limit on this device; falling back to the "
                    "in-tree Triton unified-attention kernel for the rest of "
                    "this process. See "
                    "https://github.com/vllm-project/vllm/issues/48723."
                )
                self._aiter_lds_overflow = True

        # Non-causal cross-attention (aiter is causal-only) OR a latched
        # LDS-overflow fallback: use the in-tree Triton unified kernel.
