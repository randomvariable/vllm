...
def _select_dsv4_attn_cls(vllm_config: VllmConfig) -> type[DeepseekV4Attention]:
    ...
    SM12x with FlashInfer's sparse-sm120 wrapper available, decode is routed
    through the packed ``sparse-sm120`` kernel (FlashInfer PR3395) instead of
    the FlashMLA decode kernel; everything else (packed ``fp8_ds_mla`` cache,
    metadata, prefill) is unchanged. Default off.
    """
    if (
    ...
    if envs.VLLM_DEEPSEEK_V4_FLASHINFER_SM120_DECODE:
        ...
        if (
            ...
            and has_flashinfer_sparse_mla_sm120()
        ):
