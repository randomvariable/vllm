...
"""DeepSeek V4 FlashInfer packed sparse-MLA SM120 decode (experiment, gated).
...
use FlashInfer's ``sparse-sm120`` ``BatchMLAPagedAttentionWrapper.run_sparse_mla``
kernel (FlashInfer PR3395). That kernel scales better at high concurrency in the
MTP speculative-verify (multi-query) decode shape than the FlashMLA decode
kernel, which is the root cause of the C8-C64 ctx0 decode gap measured in
``docs/sm120/experiments/2026-06-14-rtx-llm-decode-bench/``.

This path is gated behind ``VLLM_DEEPSEEK_V4_FLASHINFER_SM120_DECODE`` and is
selected only on SM12x when FlashInfer's sparse-sm120 wrapper is importable
(see ``_select_dsv4_attn_cls``). Default off; gate-off behavior is identical to
the FlashMLA decode path.
"""
...
# adapter: one tile of 64 candidates per SWA top-k plus one per compressed
# (extra) top-k. Cap the warmup reservation at the largest decode batch.
...
class DeepseekV4FlashInferSM120Attention(DeepseekV4FlashMLAAttention):
    """FlashMLA V4 attention with the packed sparse-sm120 decode kernel.
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        if not has_flashinfer_sparse_mla_sm120():
            raise RuntimeError(
                ...
                "sparse-sm120 MLA wrapper (FlashInfer PR3395)."
            )
        ...
        logger.info_once(
            "DeepSeek V4: using FlashInfer sparse-sm120 packed decode kernel "
    ...
    def _prepare_sm120_query(
        ...
        # The sparse-sm120 wrapper consumes a bf16 query; the FlashMLA fp8 path
    ...
    def _forward_decode(
        ...
        self._sm120_wrapper.run_sparse_mla(
            ...
            extra_sparse_indices=topk_indices,
            extra_sparse_lengths=topk_lens,
            mid_out=mid_out,
            mid_lse=mid_lse,
        )
