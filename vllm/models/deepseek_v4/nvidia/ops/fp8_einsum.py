...
def deepseek_v4_sm12x_fp8_einsum_quant(
    ...
    if a_scale.dtype in (torch.uint8, e8m0_dtype):
        a_scale = _upcast_e8m0_to_fp32(a_scale)
    if b_scale.dtype in (torch.uint8, e8m0_dtype):
...
def _use_deepseek_v4_sm12x_triton_fp8_einsum(
    ...
    e8m0_dtype = getattr(torch, "float8_e8m0fnu", None)
    # The B12X weight path stores raw E8M0 scale bits as uint8 (or the native
    # float8_e8m0fnu dtype when available); both are exponent-only and upcast
    # to fp32 before the kernel runs.
    scale_dtypes = (torch.float32, torch.uint8)
    if e8m0_dtype is not None:
        scale_dtypes += (e8m0_dtype,)
    return (
...
def deepseek_v4_fp8_einsum(
    ...
    if equation == "bhr,hdr->bhd" and b.dim() in (2, 3):
        ...
        if b.dim() == 2:
            ...
            # Already reshaped to 3D (g, r, d) by the is_bmm weight path.
            b_groups, b_out, b_hidden = b.shape
            if b_out != out_rank or b_hidden != hidden_size:
                raise RuntimeError(
                    "DeepSeek V4 fp8 einsum weight shape mismatch: got "
                    f"weight={tuple(b.shape)}, expected "
                    f"({b_groups}, {out_rank}, {hidden_size})"
                )
        ...
        if _use_deepseek_v4_sm12x_triton_fp8_einsum(equation, recipe, b_scale):
            ...
            init_logger(__name__).warning(
                "dsv4 fp8 einsum NOT using SM12x Triton path: "
                "equation=%s recipe=%s b.dim=%d b_scale.dim=%d b_scale.dtype=%s "
                "cap=%s a.dtype=%s b.dtype=%s",
                equation, tuple(recipe), b.dim(), b_scale.dim(),
                b_scale.dtype, current_platform.get_device_capability(),
                a.dtype, b.dtype,
            )
