...
def get_checksums(subdirs):
    ...
    for subdir in subdirs:
        ...
        if not download_file(uri, str(checksum_path)) and not checksum_path.is_file():
...
def download_artifacts() -> None:
    ...
    # Keep valid files in place so a persistent build cache avoids re-downloading
    # the complete artifact manifest on every wheel build.
    ...
    if not all(results or [True]):
        raise RuntimeError("Failed to download cubins")

    # Check checksums of all downloaded and reused artifacts.
    for name, checksum in cubin_files:
        local_path = FLASHINFER_CUBIN_DIR / name
