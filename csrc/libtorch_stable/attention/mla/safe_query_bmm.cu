...
void check_bf16_cuda_3d(torch::stable::Tensor const& tensor, const char* name) {
...
void safe_mla_query_bmm(torch::stable::Tensor const& query,
  ...
  STD_TORCH_CHECK(output.size(0) == heads_i64 && output.size(1) == tokens_i64 &&
  ...
  check_cublas(cublasSetStream(
                   handle, get_current_cuda_stream(query.get_device_index())),
               "cublasSetStream");
