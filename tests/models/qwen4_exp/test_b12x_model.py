...
def test_b12x_gdn_binds_live_projections_and_stages_rollback_metadata(
    ...
    bind_mock = Mock(wraps=bind)
    layer._b12x_decode_plan = plan
    ...
    layer._b12x_decode_staging = SimpleNamespace(
        **{
            name: getattr(layer, "_b12x_" + name)
            for name in (
                "mixed_qkv",
                "a",
                "b",
    ...
    monkeypatch.setattr(
        gdn_module, "get_b12x_scratch_buffers", lambda _: (layer._b12x_scratch,)
    )
    layer._b12x_gdn_api = SimpleNamespace(run=run, bind=bind_mock)
