...
def test_minimax_m3_qknorm_rope_uses_custom_op_while_compiling(
    ...
    q_fp8_out = torch.empty(2, 16, dtype=torch.float8_e4m3fn)
    ...
    assert seen_args[20] is q_fp8_out
    assert seen_args[21] == 0.5
...
def test_minimax_m3_sparse_attention_compile_path_avoids_python_slot_mapping(
    ...
    layer.impl = SimpleNamespace(should_use_msa_decode=lambda _: False)
    ...
    assert sparse_attn_calls[0][1] is None
    assert sparse_attn_calls[0][7] is not None
...
def test_minimax_m3_sparse_attention_custom_op_uses_forward_context() -> None:
    ...
    class RecordingImpl:
        ...
        def forward(
            ...
            *,
            ...
            self.calls.append((layer, query, kv_cache, output, query_fp8))
            output.copy_(query)
            return output
    ...
    seen_layer, seen_query, seen_cache, seen_output, seen_query_fp8 = layer.impl.calls[
        0
    ]
