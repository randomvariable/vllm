...
class _Provider:
    ...
    def get_b12x_preparation_units(self, layer, workload):
        ...
        return (
            _unit(
                f"family-{id(layer):x}", _Request(f"{id(layer):x}"), stage=self.stage
            ),
        )
...
def _worker(model, *, draft=None, draft_lane=0):
    return SimpleNamespace(
        ...
        model_runner=SimpleNamespace(
            mm_registry=None, _draft_workspace_lane=draft_lane
        ),
    )
...
def test_collective_describers_reject_process_local_identity(
...
def test_batches_group_timed_requests_before_default_only() -> None:
    ...
    batches = b12x_prepare.b12x_batches(
        [
            _unit("x", timed_a),
            _unit("y", default, autotune=False),
            _unit("z", timed_b),
        ]
    )
...
def test_collect_units_filters_by_stage_and_tunes_eager_shapes() -> None:
    ...
    assert [unit.requests[0].name for unit in units] == [
        f"{id(state):x}",
        f"{id(weights):x}",
    ]
...
def test_qwen_vision_preparation_uses_encoder_and_connector_token_counts(
    ...
    monkeypatch.setattr(
        encoder_budget,
        "MultiModalBudget",
        lambda *args, **kwargs: SimpleNamespace(get_encoder_budget=lambda: 8192),
    )

    b12x_prepare.mark_b12x_eager_shapes(worker)

    for module in model.visual.blocks.modules():
        assert module.b12x_eager_token_counts == (32768,)
        assert module.b12x_eager_only
    for module in (model.visual.merger, *model.visual.deepstack_merger_list):
        assert module.b12x_eager_token_counts == (8192,)
        assert module.b12x_eager_only
...
def test_collect_units_asks_unit_providers_and_rejects_duplicate_names() -> None:
    ...
    try:
        ...
        assert [
            unit.name
            for unit in b12x_prepare.collect_b12x_units(_worker(model), _workload())
        ] == ["comm"]
        layer = torch.nn.Module()
        layer.b12x_preparation_provider = SimpleNamespace(
            get_b12x_preparation_units=lambda module, workload: (
                _unit("dup", _Request("shared")),
            )
        )
        model.layer = layer
        with pytest.raises(
            ValueError, match="conflicting b12x preparation request names"
        ):
            b12x_prepare.collect_b12x_units(_worker(model), _workload())
    finally:
        _B12X_UNIT_PROVIDERS[:] = [
            ref for ref in _B12X_UNIT_PROVIDERS if ref() is not comm
        ]
    ...
    class _LaneProvider:
        def get_b12x_preparation_units(self, layer, workload):
            ...
            def prepare(state):
                ...
                return _Call(
                    run=lambda: observed.append(("run", _workspace_lane.get()))
                )
...
def test_blockscaled_holder_declares_provided_workspace_under_the_cap(
    ...
    monkeypatch.setattr(
        "vllm.model_executor.kernels.linear.b12x_blockscaled.get_b12x_blockscaled",
        lambda: api,
    )
    ...
    assert (
        queries[-1]["workspace_nbytes"]
        == envs.VLLM_B12X_BLOCKSCALED_WORKSPACE_MAX_BYTES
    )
    assert (
        queries[-1]["num_tokens"] == 32 and queries[-1]["global_scale_kind"] == "none"
    )
    ...
    plan = holder.plan
    again = holder.unit(
        _workload(fixed_token_counts=(1, 2, 4)), name="linear.mxfp8.layer"
    )
    ...
    tower = B12xBlockscaledLinear(
        packed, recipe="mxfp8", activation_mode="auto", layer_name="tower"
    )
    eager = tower.unit(
        _workload(
            token_counts=(65_536,),
...
def test_profile_prepares_collectives_and_releases_only_fresh_plans(
    monkeypatch, fail
) -> None:
    ...
    plain, collective = (
        _Request("plain"),
        _Request("collective", collective=requirement),
    )
    installed = _Request(
    ...
    monkeypatch.setattr(
        workspace, "_manager", workspace.WorkspaceManager(torch.device("cpu"))
    )
    ...
    def begin(requests, *, autotune):
        prepared.append((requests, autotune))
        return Job()
    ...
    monkeypatch.setattr(
        b12x_prepare, "b12x_workload", lambda worker, *, stage, lane=0: _workload(stage)
    )
    monkeypatch.setattr(
        b12x_prepare,
        "collect_b12x_units",
        lambda worker, workload: [
            _unit("a", plain, stage="state"),
            _unit("b", collective, stage="state"),
    ...
    world = SimpleNamespace(
        ranks=(0,),
        tcp_store_group=SimpleNamespace(all_gather_obj=lambda payload: [payload]),
    )
...
def _config_worker(
    *, capture_sizes, max_seqs, speculative_tokens, decode_query_len=None, manager=None
):
    ...
    spec = (
        ...
        else SimpleNamespace(num_speculative_tokens=speculative_tokens)
    )
    runner = SimpleNamespace(
        mm_registry=None, _draft_workspace_lane=0, cudagraph_manager=manager
    )
    if decode_query_len is not None:
        runner.decode_query_len = decode_query_len
    return SimpleNamespace(
        vllm_config=SimpleNamespace(
            compilation_config=compilation, speculative_config=spec
        ),
        scheduler_config=SimpleNamespace(
            max_num_batched_tokens=128, max_num_seqs=max_seqs
        ),
...
def test_workload_declares_the_same_decode_counts_in_both_stages() -> None:
    sizes = (1, 2, 4, 8, 16)
    before = _config_worker(
        capture_sizes=sizes, max_seqs=2, speculative_tokens=3, decode_query_len=4
    )
    ...
    assert b12x_prepare.b12x_workload(implicit, stage="weights").token_counts == (
        1,
        2,
        3,
        4,
    ...
    assert b12x_prepare.b12x_workload(plain, stage="weights").token_counts == (
        1,
        2,
        4,
        128,
...
def test_declaration_dump_flattens_metadata_and_never_raises(
    ...
    contract = SimpleNamespace(
        component_id="test.family",
        query_schema_version=3,
        encode_query=lambda query: {
            "rows": query.rows,
            "dtype": torch.bfloat16,
            "recipe": FrozenMapping({"tile": (64, 128)}),
        },
    )
    plan = SimpleNamespace(
        contract=contract,
        query=SimpleNamespace(rows=4),
        invocation=FrozenMapping(
            {"operation": "pre", "nested": FrozenMapping({"k": 1})}
        ),
        shared=True,
    )
    ...
    broken = SimpleNamespace(
        contract=SimpleNamespace(
            ...
            encode_query=lambda q: (_ for _ in ()).throw(RuntimeError("boom")),
        ),
        query=None,
        invocation=FrozenMapping(),
        shared=False,
    )
    b12x_prepare._dump_declarations(
        [_unit("broken", _Request("b", plan=broken))], _workload()
    )
...
@pytest.mark.parametrize("draft_lane", [0, 1])
def test_collect_units_handles_target_and_draft_embedding_aliases(
    shared_module, draft_lane
):
    ...
    target.embed_tokens.weight = torch.nn.Parameter(
        torch.ones(32, 64, dtype=torch.bfloat16),
        requires_grad=False,
    )
...
def test_parallel_draft_counts_reach_owners_and_collectives(monkeypatch, query_len):
    ...
    assert (
        draft.b12x_preparation_provider.record[-1][1].token_counts
        == draft_workload.token_counts
    )
...
def test_workload_prepares_every_adaptive_verification_profile_shape():
    ...
    worker = _config_worker(
        capture_sizes=capture_sizes, max_seqs=8, speculative_tokens=7
    )
    ...
    profiled = {
        batch["num_tokens"] for batch in manager.batches_to_profile(list(capture_sizes))
    }
...
def test_workload_covers_sampler_warmup_prefill_request_limits():
    ...
    for stage in ("weights", "state"):
        workload = b12x_prepare.b12x_workload(worker, stage=stage)
        assert all(
            prompt_len * reqs in workload.token_counts
            for reqs in range(1, max_reqs + 1)
        )
