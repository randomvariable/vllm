...
def test_indexer_group_selector_supports_partial_target_and_sharded_draft(
    ...
    partial_dcp = SimpleNamespace(world_size=4)
    configured_dcp = SimpleNamespace(world_size=8)
    partial_query_split = SimpleNamespace(world_size=2)
    configured_query_split = SimpleNamespace(world_size=1)
    monkeypatch.setattr(parallel_state, "_INDEXER_DCP", partial_dcp)
    monkeypatch.setattr(parallel_state, "_DCP", configured_dcp)
    monkeypatch.setattr(parallel_state, "_INDEXER_QUERY_SPLIT", partial_query_split)
    monkeypatch.setattr(parallel_state, "_QUERY_SPLIT", configured_query_split)

    assert parallel_state.get_indexer_dcp_group(4) is partial_dcp
...
def test_indexer_group_selector_rejects_unknown_shard_count(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="requested=2"):
        parallel_state.get_indexer_query_split_group(2)
