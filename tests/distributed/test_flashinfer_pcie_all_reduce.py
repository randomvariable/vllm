...
def test_flashinfer_ipc_does_not_bypass_multinode_world_size_guard(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    load_pool = MagicMock()
    ...
    monkeypatch.setattr(
        custom_all_reduce,
        "in_the_same_node_as",
        lambda group, source_rank=0: [True, False, False],
    )
    monkeypatch.setattr(custom_all_reduce.dist, "get_rank", lambda group=None: 0)
    monkeypatch.setattr(custom_all_reduce.dist, "get_world_size", lambda group=None: 3)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        custom_all_reduce, "_load_flashinfer_pcie_oneshot_pool", load_pool
    )
