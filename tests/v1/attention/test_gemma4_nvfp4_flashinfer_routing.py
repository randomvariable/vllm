...
class _LayeredArchConfig:
    """Minimal stand-in for the fork's per-layer arch config: supports
    len-style iteration via total_num_hidden_layers and int indexing."""

    def __init__(self, head_dim: int, global_head_dim: int) -> None:
        self.total_num_hidden_layers = 6
        self._layers = [
            SimpleNamespace(head_size=head_dim),
        ] * 5 + [SimpleNamespace(head_size=global_head_dim)]
    ...
    *, backend=None, cache_dtype="nvfp4", head_dim=256, global_head_dim=512
    ...
    return SimpleNamespace(
        ...
        model_config=SimpleNamespace(
            ...
            model_arch_config=_LayeredArchConfig(head_dim, global_head_dim),
        ),
    )
